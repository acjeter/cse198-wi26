---
layout: default
title: How to RSVP
parent: Guides
nav_order: 1
permalink: /docs/guides/portal-rsvp.html
---

# How to RSVP on the HKN Portal

The [HKN Outreach Portal](https://portal.hknucsd.com/) is the single source of truth for all events.

## Step 1: Portal Access

{: .important-title}
> **Important**
>
> When you **FIRST access the portal**, use the **specific sign up link in Discord**. For any other time, use the link: [HKN Outreach Portal](https://portal.hknucsd.com/).

## Step 2: Find an Event
1. Navigate to the "Events" tab.
2. Toggle only the "Outreach" filter to see only CSE/ECE/MAE 198 events and On-Campus Tours.
3. Look for events tagged with **CSE/ECE/MAE 198** or general Outreach.

## Step 3: RSVP
1. Click the "RSVP" button to indicate that you plan to attend the event.
2. Confirm that you've completed the associated content quiz, if you are not a creator of the lesson.

## Step 4: Check-In
1. **One to two days before the event**, rides will be assigned in the `#rides` channel on Discord.
2. Please view [rides](rides.html) for what to expect during ride assignment.

{: .important-title }
> **Important**
>
> **Signing up commits you to attending.** If you need to cancel, **please do so at least 24 hours** in advance so others can fill your ride spot.